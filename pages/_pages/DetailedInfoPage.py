...
class DetailedInfoPage:
    ...
    def __build_page_layout(self, df, hist_cols):
        ...
        if len(deck) > 0:
            ...
            with header_col[2]:
                ...
                st.plotly_chart(self.vis.plot_metric(label=f"Aktuelle Elo", 
                ...
                inside[0].metric(f"Aktueller Rang", value=f'{platz}', delta=f'Besser als {percentage}%')
                		
                ds = pd.Series(df.loc[idx_deck, hist_cols+['Elo']].values)
                ds = ds[ds > 0].reset_index(drop=True)
                
                inside[2].metric(f"Beste Elo", value=f"{int(df.loc[idx_deck, hist_cols+['Elo']].max())}",
            ...
            col = st.columns([2,5])
            # wanderpokal
            wanderpokal = ''.join(':trophy: ' for i in range(int(df.at[idx_deck, 'Meisterschaft']['Win']['Wanderpokal'])))
            col[0].markdown('Wanderpokal '+wanderpokal)
            # local wins
            localwin = ''.join(':medal: ' for i in range(int(df.at[idx_deck, 'Local Win'])))
            col[0].markdown('Local Win '+ localwin)
            #
            funpokal = ''.join(':star: ' for i in range(int(df.at[idx_deck, 'Meisterschaft']['Win']['Fun Pokal'])))
            col[0].markdown('Fun Pokal '+funpokal)
            #
            localtop= ''.join(':star: ' for i in range(int(df.at[idx_deck,'Local Top'])))
            col[0].markdown('Local Top '+localtop)
    ...
    def __line_plot(self, df, hist_cols, idx):
        ...
        fig = self.vis.lineplot(df_plot["Datum"], df_plot["Elo"], df_plot["Mittelw."])
        return fig
