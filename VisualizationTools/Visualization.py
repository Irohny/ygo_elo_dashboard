...
def plotly_gauge_plot(value, title="Gewinnrate [%]"):
    """
...
def lineplot(x_data, y_data, mean):
    """
    ...
    fig.update_layout(
        ...
        xaxis_title="Datum",
        yaxis_title="Elo",
        legend=dict(yanchor="top", y=1, xanchor="left", orientation="h", x=0),
    )
    return fig


def make_spider_plot(stats, categories: list[str] = gv.DECK_ATTRIBUTES):
    """
    Method to mmake a spider plot to show deck stats
    :param stats: list with stats of the current deck
    return fig: plotly figure object
    """
    # append first element for cloesed spider plot
    ...
    plot = go.Scatterpolar(
        ...
        theta=categories,
        fill="toself",
        line=dict(color=gv.PRIMARY_COLOR, width=3),
    )
...
def __semi_circle_plot(val):
    ...
    colors = ["green", "blue", "red", gv.BACKGROUND_COLOR]
    ...
    ax.text(
        -1.05,
        1.1,
        f"N {int(val[2]/(0.5*np.sum(val)+1e-7)*1000)/10}%",
        color="white",
        fontsize=14,
    )
    ax.text(
        0.65,
        1.1,
        f"S {int(val[0]/(0.5*np.sum(val)+1e-7)*1000)/10}%",
        color="white",
        fontsize=14,
    )
    ax.add_artist(plt.Circle((0, 0), 0.6, gv.BACKGROUND_COLOR))

    return fig


def ploty_bar(df, xfeat, yfeat, horizontal=True, title=None):
    """
...
def box_violin_plot(
    ...
    for idx, tmp_type in enumerate(categories):
        axs.violinplot(
            df[df[categorie] == tmp_type][distribution],
            positions=[idx],
            showmeans=True,
            showextrema=False,
            showmedians=False,
    ...
    axs.set_title(title, color=gv.TEXT_COLOR)
    axs.set_ylabel(ylabel, color=gv.TEXT_COLOR)
    axs.set_xticks(range(len(categories)))
    axs.set_xticklabels(categories, rotation=-45, color=gv.TEXT_COLOR)
    axs.grid()

    axs.spines["bottom"].set_color(gv.TEXT_COLOR)
    axs.spines["left"].set_color(gv.TEXT_COLOR)
    axs.tick_params(colors=gv.TEXT_COLOR, which="both")
    return fig


def stacked_bar(values):
    """ """
    ...
    fig.update_layout(
        ...
        ),  # Entfernen der x-Achsen-Beschriftungen und Rasterlinien
        yaxis=dict(
            showticklabels=False, showgrid=False, zeroline=False
        ),  # Entfernen der y-Achsen-Beschriftungen und Rasterlinien
        showlegend=False,
    )
    return fig


def tournament_date_bar_plot(df):
    """
    ...
    fig.add_trace(
        go.Bar(
            ...
            name="Losses",
            marker_color="red",
            text=df["Type"] + " " + df["Result"] + "<br>" + df["Date"].astype(str),
...
def deck_lineplot(decks: pd.DataFrame, hist_cols: str = None):
    """
    ...
    if decks.empty:
        return None
    decks = decks[decks["Elo"] > 0].reset_index(drop=True)

    elo_min = decks["Elo"].min() - 15
    elo_max = decks["Elo"].max() + 15
    decks["Datum"] = (
        decks["Datum"].astype(int).apply(date_handling.convert_integer_to_date)
    )
    decks.sort_values("Datum", ascending=False, inplace=True, ignore_index=True)
    # generate a plotly figure with elo ratings
    fig = px.line(decks, x="Datum", y="Elo", color="Deck", line_shape="spline")
    # update figure traces for hover effects and line width
    fig.update_traces(
        textposition="bottom right", line=dict(width=5), hovertemplate=None
    )
    ...
    fig.update_layout(
        ...
        yaxis_title="Elo",
        hovermode="x unified",
        yaxis=dict(range=[elo_min, elo_max]),
    )
