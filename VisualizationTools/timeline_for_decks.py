...
def timeline_for_decks():
    ...
    time_json = {
        "title": {
            "text": {"headline": "Top 5", "text": "Die besten Decks im Vergleich"}
        },
        "events": [],
    }
    ...
    for date in st.session_state["history_columns"]:
        # setup date
        datum = {"year": str(date[3:]), "month": str(int(date[:2]))}
        # set up placements
        test = (
            st.session_state["deck_data"][[date, "Deck"]]
            .copy()
            .sort_values(by=date, ascending=False)
            .reset_index(drop=True)
        )
        text = {
            "text": f"<p>{test.at[0, 'Deck']} {int(test.at[0, date])}<p>\
        ...
        event = {"start_date": datum, "text": text}
