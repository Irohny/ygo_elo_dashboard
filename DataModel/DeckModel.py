...
class DeckModel:
	...
	def __init__(self, load_data:bool=True) -> None:
		self.db = connect2deta(key=st.secrets['key'], name="YgoEloBase")
	...
	def __create_data_model(self):
		...
		df, hist_cols, self.cols = self.__get_all_data()
		self.hist_cols = self.__sort_hist_cols(hist_cols)
		self.df = self.__clean_data(df)
	...
	def __clean_data(self, df):
		...
		cols_to_int = ['Elo', 'dgp', 'Wanderpokal', 'Liga Pokal', 'Fun Pokal', 'Attack', 'Combo', 'Resilience', 'Recovery',
					'Consistensy', 'Control','Matches', 'Siege', 'Remis', 'Niederlage']+self.hist_cols
	...
	def __sort_hist_cols(self, hist_cols):
		...
		'''
		t = pd.Series([int(f"{c[-4:]}{c[:2]}") for c in hist_cols]).sort_values(ascending=True, ignore_index=True)
